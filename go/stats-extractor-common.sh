# Общая часть обёрток stats-extractor: находит env и бинарь.
# Подключается через `. "$SCRIPT_DIR/stats-extractor-common.sh"`, SCRIPT_DIR уже задан.
#
...
# там нет ни того, ни другого; ставит их `deploy.sh stats-extractor`.

ENV_FILE="$SCRIPT_DIR/getreplay-go.env"
BIN="$SCRIPT_DIR/stats-extractor"
...
not_installed_here() {
  ...
  echo >&2
  echo "    /home/solo/infra/go/deploy.sh stats-extractor" >&2
